...
struct StudentNoScheduleView: View {
    let store: StoreOf<StudentNoScheduleFeature>
    @ObservedObject var viewStore: ViewStoreOf<StudentNoScheduleFeature>
    @StateObject private var permissionManager = CameraPermissionManager()

    public init(store: StoreOf<StudentNoScheduleFeature>) {
        self.store = store
        self.viewStore = ViewStore(store, observe: { $0 })
    }
    
    var body: some View {
        StudentNoScheduleBodyView(viewStore: viewStore)
        .onAppear {
...
private struct StudentNoScheduleBodyView: View {
    private let viewStore: ViewStoreOf<StudentNoScheduleFeature>
    
    fileprivate init(viewStore: ViewStoreOf<StudentNoScheduleFeature>) {
